...
%% Parameters
clc
VNA_data = readmatrix('data.txt'); % File name of the VNA data S-parameter file [f s11r s11i s21r s21i s21r s21i s22r s22i]
t = input('thickness of the shilding material in cm:--> '); % Thickness of the Shielding material in cm
% S11=VNA_data(:,2)+1i*VNA_data(:,3);                                         % Reform S11 in the complex form
% S21=VNA_data(:,4)+1i*VNA_data(:,5);                                         % Reform S21 in the complex form
S22 = VNA_data(:, 8) + 1i * VNA_data(:, 9); % Reform S22 in the complex form
S12 = VNA_data(:, 6) + 1i * VNA_data(:, 7); % Reform S12 in the complex form
frq = VNA_data(:, 1); % Extract the frquency data
f = frq ./ 10^9; % X-band frquency in GHz
fc = 6.557; %input('Cutoff frequency in GHz:--> '); % The cutoff_frq in GHz use to calculate wavelength lamda_0
...
permittivity = (((first_term_permeability1_sqr + (1 ./ (lamda_c).^2)) .* lamda_0.^2)) ./ (permeabillity);
%% UPdate the file name
mu_cfrp8l08mm12 = permeabillity(1:end, 1); % change the veriable name and replace for all
epcilon_cfrp8l08mm12 = permittivity(1:end, 1); % change the veriable name and replace for all
f = f(1:end);
...
subplot(2, 2, 1)
plot(f, real(mu_cfrp8l08mm12(1:end - 1)), 'LineWidth', 2)
hold on
% ylim([0 60])
